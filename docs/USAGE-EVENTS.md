...
| `source` | text | `bridge.mcp`, `bridge.checkout`, `agent.onboarding`, `agent.catalog`, `agent.guardian` (one row per checkout where a rule fired; `model` null when the rule spoke without a model), `simulator` |
