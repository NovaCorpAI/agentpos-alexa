...
describe("usage_events export", () => {
  it("writes the documented columns in order, quotes what needs quoting, and round-trips through SQLite", () => {
    ...
    storage.usageEvents.record({ traceId: "t3", source: "agent.catalog", storeOrigin: "http://bakery.test", checkoutSessionId: "cs_1", model: "amazon.nova-2-lite-v1:0", inputTokens: 812, outputTokens: 96, cacheReadTokens: 2318, cacheWriteTokens: 4, latencyMs: 640, estimatedCostUsdMicros: 74, simulated: false, onboardingStage: undefined as never });
    ...
    expect(lines[0]).toContain("cache_read_tokens,cache_write_tokens");
    expect(lines[2]).toContain("com.amazon.payments.network_token/amazon_pay_network_token");
    expect(lines[3]).toContain(",amazon.nova-2-lite-v1:0,812,96,2318,4,640,74,");
